...
public class MemberRepository {
	...
	public MemberRepository() throws FileNotFoundException {
		String curDir = System.getProperty("user.dir");
		path = curDir + "/bin/database/members.txt";
	}
	 ...
	 * [6] = zip
	 */
	public Member getMemberFromString(String str) {
		String[] data = readDataString(str);
		return new Member(Integer.parseInt(data[1]), data[2], data[3], data[4], data[5], Integer.parseInt(data[6]));
	}
	
	public String memberToString(Member m) {
		return "Member: " + m.getId() + " \"" + m.getName() + "\" \"" + m.getAddress() + "\"" + 
				" \"" + m.getCity() + "\" \"" + m.getState() + "\" " + m.getZip() + "\n";
	}
	
	/********** core functions ********************/
	public int save(Member m) throws IOException {
		String data = memberToString(m);
		append(data);
		return m.getId();
	}
	
	public Member findById(int id) throws FileNotFoundException, IOException {
		return this.getMemberFromString(this.getDataStringById(id));
	}
	...
	public Member update(Member m) throws FileNotFoundException, IOException {
		String data = memberToString(m);
